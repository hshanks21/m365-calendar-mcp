...
Confidential account and optional Supabase project pins are mandatory trusted-operator deployment configuration, not source edits or caller inputs. `CALENDAR_M365_DELEGATED_EXPECTED_USERNAME` binds cache, returned identity and work-policy mailbox; `CALENDAR_SUPABASE_ALLOWED_ORIGIN` binds the separately configured URL, returned authorization origin and exact auth routes. Both validate offline with no default. One reviewed image can be promoted unchanged across approved deployments; app-only and Google behavior is unchanged. See M365_RUNTIME.md and M365_SUPABASE.md.
