...
- Mandatory trusted-operator identity/origin pins must be injected at runtime (bootstrap-only pins only for bootstrap), with complete offline fail-closed validation before listeners. Promote one immutable image across approved deployments without private source edits; never expose pin changes as agent tools or derive approval from returned URLs/accounts.
