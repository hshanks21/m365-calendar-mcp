...
No running-service restart, registry push, tag, CI workflow or live-provider request is performed by these credential-free build instructions. Tag-only GHCR publication is now implemented, but rollout/rollback still use the existing approved host operator by digest, never an automatic replacement from this checkout. Imported confidential/Supabase pins are invalid templates and must be reviewed before deployment. See [MCP_DEPLOYMENT_STANDARD.md](MCP_DEPLOYMENT_STANDARD.md) and [../RELEASING.md](../RELEASING.md).
