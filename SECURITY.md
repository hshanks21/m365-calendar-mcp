...
Personal account/project pins in the public import are `example.invalid` placeholders that retain exact-match validation. Do not replace them with wildcard acceptance, caller input or a generic credential fallback. Review and test any deployment-specific pin change independently. Operator bootstrap Doppler cwd/scope is pinned to the executable checkout; the installed sibling retains its existing scope. Repository publication does not configure or migrate a secret-manager identity.
